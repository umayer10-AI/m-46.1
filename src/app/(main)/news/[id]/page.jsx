import Image from 'next/image';
import Link from 'next/link';
import React from 'react';
import { FaArrowLeftLong } from 'react-icons/fa6';

const fetching = async (myid) => {
    const res = await fetch(`https://openapi.programming-hero.com/api/news/${myid}`)
    return res.json()
}

const page = async ({params}) => {

    const {id} = await params
    const f = await fetching(id)
    const data = f.data[0]
    console.log(data)

    return (
        <div className='max-w-[80%] mx-auto'>
            <h2 className='text-xl font-bold mb-3'>Dragon News</h2>
            <div className='space-y-3'>
                <img src={data.image_url} alt="logo" className='w-full h-80 rounded-2xl object-cover' />
                <h2 className='text-2xl font-bold'>{data.title}</h2>
                <h2 className='line-clamp-5'>{data.details}</h2>
                <Link href={'/category/01'} className='btn btn-error text-white bg-red-600'><FaArrowLeftLong /> All news in this category</Link>
            </div>
        </div>
    );
};

export default page;